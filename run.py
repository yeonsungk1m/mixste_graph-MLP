...
from common.regularizations import sagittal_symmetry, segments_time_consistency
...
print('INFO: Trainable parameter count:', model_params/1000000, 'Million', flush=True)
if not args.nolog:
...
if not args.evaluate:
    ...
    train_generator_eval = UnchunkedGenerator_Seq(cameras_train, poses_train, poses_train_2d,
                                              pad=pad, causal_shift=causal_shift, augment=False)
    print('INFO: Training on {} frames'.format(train_generator_eval.num_frames()), flush=True)
    if not args.nolog:
    ...
    for epoch in tqdm(range(epoch, args.epochs), desc="epoch"):
        start_time = time()
        ...
        if args.no_eval:
            ...
            print('[%d] time %.2f lr %f 3d_train %f 3d_eval %f 3d_valid %f 3d_val_velocity %f' % (
                ...
                epoch_loss_3d_vel * 1000), flush=True)
            if not args.nolog:
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            generated_poses = predicted_3d_pos.permute(0,3,2,1)
            mpsse = (
                sagittal_symmetry(
            ...
            epoch_mpsce += inputs_3d.shape[0]*inputs_3d.shape[1] * mpsse
            epoch_mpsse += inputs_3d.shape[0]*inputs_3d.shape[1] * mpsce
    ...
    mpsce = 1000 * epoch_mpsce / N
    mpsse = 1000 * epoch_mpsse / N

    print('----------')

    return e1, e2, e3, ev, mpsce, mpsse
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            e1, e2, e3, ev, mpsce, mpsse = evaluate(gen, action_key)
            ...
            errors_mpsce.append(mpsce)
            errors_mpsse.append(mpsse)

        print('Protocol #1   (MPJPE) action-wise average:', round(np.mean(errors_p1), 1), 'mm')
        print('Protocol #2 (P-MPJPE) action-wise average:', round(np.mean(errors_p2), 1), 'mm')
        print('Protocol #3 (N-MPJPE) action-wise average:', round(np.mean(errors_p3), 1), 'mm')
        print('Velocity      (MPJVE) action-wise average:', round(np.mean(errors_vel), 2), 'mm')
        print(f"MPSCE: {np.mean(errors_mpsce):.1f}")
        print(f"MPSSE: {np.mean(errors_mpsse):.1f}")
        print(f"", flush=True)
